...
def get_blobs(tvts = [0.7,0.15,0.15]):
  data = blobs(300, [np.array([1, 2]), np.array([3, 4])], [np.array([[0.5, 0.5], [0.125, 0.25]])] * 2).to_numpy()
  l = data.shape[0]
  tx = data[0:int(l*tvts[0]),:2]
  ty = data[0:int(l*tvts[0]),2:].astype(int)
  tvx = data[int(l*tvts[0]):int(l*tvts[0])+int(l*tvts[1]),:2]
  tvy = data[int(l*tvts[0]):int(l*tvts[0])+int(l*tvts[1]),2:].astype(int)
  tex = data[int(l*tvts[1]):int(l*tvts[1])+int(l*tvts[2]),:2]
  tey = data[int(l*tvts[1]):int(l*tvts[1])+int(l*tvts[2]),2:].astype(int)
